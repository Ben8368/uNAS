...
> 正式产品名：uNAS。当前阶段与验证范围见 [CONTEXT.md](CONTEXT.md)。除已授权目录中的受限 ZIP 解压探针外，本地 Demo 使用固定模拟数据，不读取真实文件或执行真实转换。
...
前端 Demo 的依赖已锁定在 workspace；除文件管理中已实现、但仍待完整 SP-04 验收的受限 ZIP 解压外，界面展示的文件、任务和处理结果均为 mock，不代表任何格式支持、真实输出或性能已验证。
