...
- ZIP：当前受限解压路径仅接受已授权目录中的单个 `.zip`，先检查 ZIP magic，再在 Dedicated Worker 中以严格模式读取；拒绝路径穿越、绝对路径、反斜杠路径、Windows 保留名、软链接、加密条目、重复/文件目录冲突、超过 200 项、单项 32 MiB、输入 50 MiB 或展开总量 64 MiB。CRC 校验和全部输出暂存完成后才创建新的同级目录；不覆盖既有项目。该实现仍需 SP-04 的完整负向夹具、目标 Chrome 人工、取消/页面关闭与资源测量证据，不能据此承诺通用 ZIP 支持。
