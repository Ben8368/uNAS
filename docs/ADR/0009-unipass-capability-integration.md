...
uNAS 现在能在单一 MV3 包内初始化 DNR、Vault 同步、Legacy 兼容和原版浮层；删除 Legacy Adapter 不影响 WebDAV Vault、AdBlock、New Tab 和浮层的模块依赖方向。代价是当前包增加 WASM、外部主机权限和独立安全审计范围；UniPass 资源许可证、uNAS 最终签名 key、真实 WebDAV/Legacy 账号和 Chrome 工具栏人工截图仍需发布前证据。
