...
规则以 [DESIGN_SYSTEM](DESIGN_SYSTEM.md) 的 UI 编号为准；本文只维护检查方法和证据。新增/修改页面检查受影响范围；共享 token、材料或组件变更检查全部受影响消费方，等价页面可分组并说明抽样理由。

| 检查 | 最低证据 |
| --- | --- |
| 规范映射 | 基线版本、UI 编号、组件/token 复用情况；例外附理由与维护者接受记录 |
| 跨 App 复用 | 文件管理与迁移 App 在同视口、主题及窗口状态下对照尺寸、标题栏、间距和控件；源码确认共用窗口/布局组件，非复制 CSS；槽位省略无留白，功能与权限行为不串用 |
| 骨架与响应式 | 1440×900、1024×768、390×844 CSS px 下受影响页面截图；涉及阈值时检查 699/700、1099/1100 两侧，App 另验小容器 |
| 主题与材料 | 浅/深主题、浅/深壁纸；玻璃组件检查减少透明度、高对比、blur 回退，附实际对比度结果 |
| 组件与流程状态 | 适用的 default/hover/pressed/focus/disabled/loading，以及空态、错误、权限、进度、取消和结果；不适用项写原因 |
...
私有预览按 [ADR 0008](ADR/0008-private-preview-modern-chrome.md) 集中验收一个固定的维护者 Chrome Stable 环境，不要求旧版或其他浏览器矩阵；已接受的环境证据从 [CONTEXT](../CONTEXT.md) 跳转对应验收记录，UI 布局测试尺寸见第 5 节。新基线迁移及后续 Chrome 更新须重跑核心扩展流程；灰度/硬件能力仍做实际探测。每项真实验收记录：
...
Chrome DevTools MCP 是优先接入的诊断工具，仍由 Playwright 与人工验收提供可重复的行为证据，接入工作见 FE-12。证据需记录工具/Chrome 版本、构建标识、profile 隔离方式、操作步骤和实际输出；截图或工具“成功”不能代替行为断言。Service Worker 生命周期必须包含未附加调试器的休眠、唤醒与关闭对照，避免调试会话延长寿命而掩盖问题；调试日志、trace 不得夹带真实用户数据或自动上传。
...
- Frontend 工作引用设计基线/UI 编号，明确复用组件、适用状态、证据和例外；按第 5 节验收。
...
- 组件、mock flow、扩展 E2E 完成，且第 5 节 UI 验收证据与例外已核对。
- 未验证真实能力仍保持计划/模拟状态。
