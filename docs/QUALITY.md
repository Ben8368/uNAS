...
私有预览按 [ADR 0008](ADR/0008-private-preview-modern-chrome.md) 集中验收一个固定的维护者 Chrome Stable 环境，不要求旧版或其他浏览器矩阵；精确版本、验证 OS 和参考视口由 RISK-001 跟踪，在 G1 前确认。新基线迁移及后续 Chrome 更新须重跑核心扩展流程；灰度/硬件能力仍做实际探测。每项真实验收记录：
