...
共享 Token 与窗口材质是各工作包的前置条件；范围、视觉规则和组件状态仍以 [DESIGN_SYSTEM](DESIGN_SYSTEM.md) 为准。

| ID | 工作 | 完成条件 |
| --- | --- | --- |
| UI-Glass-B | Desktop、Dock 与启动器 | Dock/启动器只使用导航玻璃；搜索采用共享 Field 状态；App 名称可两行显示或查看全名；在 1440×900、1024×768、390×844 下完成键盘、焦点与空态验收。 |
| UI-Glass-C | 文件管理器试点 | 侧栏、地址/搜索栏、操作、列表选择与底部状态区消费共享 Token/控件；内容面保持实色；目录授权、写入确认、删除和移动语义及 contract 不变。 |
| UI-Glass-D | UniPass 展示层适配 | 独立 Popup/浮层通过适配层映射共享材质、间距、输入、弹窗和焦点语义；保留薄荷绿品牌强调；不改凭据、认证、WASM、网络或权限代码。 |
| UI-Glass-E | 视觉与可访问性验收 | 对 B–D 在深浅主题、减少透明度、高对比、减少动态、窄窗口、200% 缩放、键盘和真实 Chrome 解包扩展中留存可复现证据；`pnpm verify` 与 `pnpm test:e2e` 均通过。 |

每个工作包完成后移除其替代的私有样式；全局兼容覆盖的偿还跟踪见 [TD-002](TECH_DEBT.md#td-002全局兼容样式的局部化迁移)。
...
- 复用受限解压 UI/port/Worker，在隔离 Profile 与 OPFS 子目录中运行自生成小型 ZIP；记录来源、许可、哈希和预期，不使用用户文件或真实密码库。
- 验证伪格式、损坏 CRC、路径/加密/软链接/重复冲突与声明资源超限在提交前失败；每个失败无输出，随后合法夹具仍能成功。
- 修复可复现的预检漏洞，保持现有格式/权限范围；完整提交事务、页面关闭恢复和资源定标另拆。证据只维护在 [SP-04](../benchmarks/sp-04/README.md)。

#### SP-04-B 预提交取消与暂存清理

- 仅覆盖已暂存、尚未创建输出目录的 prepare 阶段：取消终止 Worker 并释放 owner 暂存；Files 窗口关闭请求同一取消。使用隔离 Profile、固定 OPFS 子目录与程序生成 ZIP。
- 验收直接取消、Files 窗口关闭、后续合法输入恢复，以及 Worker/回调清理；不把结果写成浏览器标签/进程终止、提交回滚或峰值内存证据。证据只维护在 [SP-04](../benchmarks/sp-04/README.md)。

#### SP-04-C 提交阶段受控写入失败

- 在隔离 OPFS 目录中使用程序生成的双文件合法 ZIP，只在测试页让第二个文件的 `createWritable()` 返回受控写入错误；不新增运行时开关、权限或用户文件依赖。
- 验收失败进入可读错误态、报告已写入文件数、保留可解释的部分输出，随后恢复原生写入并在唯一新目录完整重试；不把部分输出写成原子回滚或通用写入失败保证。证据只维护在 [SP-04](../benchmarks/sp-04/README.md)。
