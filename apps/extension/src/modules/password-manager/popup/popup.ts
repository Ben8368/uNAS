...
export function initializePopup(environment: PopupEnvironment = {}): PopupHandle {
  ...
  function applyOnlineSession(user: PopupSessionUser): void {
    ...
    if (!userScope) setStatus("uNAS 会话缺少稳定用户标识，请联系管理员", true);
    ...
    sessionBadge.setAttribute("aria-label", nickName ? `昵称：${nickName}；点击打开密码管家应用页` : "点击打开密码管家应用页");
  }
  ...
  function applyOfflineSession(error: unknown): void {
    ...
    sessionBadge.disabled = false;
    sessionBadge.title = "一键登录密码管家，并授权 Tec-IAM 获取飞书身份标识";
    sessionBadge.setAttribute("aria-label", "一键登录密码管家，并授权 Tec-IAM 获取飞书身份标识");
  ...
  function beginLoginRefresh(): void {
    ...
    sessionBadge.title = "正在后台登录并确认 uNAS 会话";
    sessionBadge.setAttribute("aria-label", "正在后台登录并确认 uNAS 会话");
    setStatus("正在后台登录，等待 uNAS 会话确认");
