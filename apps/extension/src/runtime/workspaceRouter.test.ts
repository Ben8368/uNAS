...
describe('legacy Workspace routing boundary', () => {
  ...
  it('rejects even valid old launch messages without creating or focusing tabs', () => {
    const respond = vi.fn(); const create = vi.fn(); const update = vi.fn()
    let listener: (...args: any[]) => unknown = () => {}
    vi.stubGlobal('chrome', { runtime: { id: 'unas', onMessage: { addListener: (fn: typeof listener) => { listener = fn } } }, tabs: { create, update } })
    try {
      installWorkspaceRouter()
      listener(launch, sender, respond)
      expect(respond).toHaveBeenLastCalledWith({ ok: false, error: expect.stringContaining('刷新旧的') })
      listener(launch, { ...sender, id: 'other' }, respond)
      expect(respond).toHaveBeenLastCalledWith({ ok: false, error: expect.stringContaining('无效') })
